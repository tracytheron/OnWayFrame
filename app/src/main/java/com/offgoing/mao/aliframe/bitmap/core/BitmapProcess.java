...
public class BitmapProcess {
	...
	public Bitmap getBitmap(String url, BitmapDisplayConfig config) {
		...
		if(null == bitmap){
			bitmap = getFromSd(url, config);
			if (null == bitmap) {
				byte[] data = mDownloader.download(url);
				if(data != null && data.length > 0){
					bitmap =  BitmapDecoder.decodeBitmapFromByteArray(data, 0, data.length, config);
					
					if (config.isCache()) {
						mCache.addToDiskCache(url, data);
						mCache.addToSd(url, data);
					}
				}
			}
		}
	...
	public Bitmap getFromDisk(String key, BitmapDisplayConfig config) {
        ...
        Bitmap b = null;
        try {
        	boolean found = mCache.getImageDataFromDisk(key, buffer);
            if (found && buffer.length - buffer.offset > 0) {
            	b = BitmapDecoder.decodeBitmapFromByteArray(buffer.data,buffer.offset, buffer.length ,config);
            }
        } finally {
	...
	public Bitmap getFromSd(String key, BitmapDisplayConfig config) {
		return mCache.getImageDataFromSd(key, config);
	}
}
